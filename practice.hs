...
--    handle <- openFile "parqueadero.txt" WriteMode
--    hputStrLn handle (unlines (map mostrarVehiculo parqueadero))
...
cargarParqueadero = do
    handle <- openFile "parqueadero.txt" ReadWriteMode
    contenido <- hGetContents handle
    contenido `deepseq` hClose handle
...
cicloPrincipal parqueadero = do
    ...
    putStrLn "4. Mostrar parqueadero"
    putStrLn "5. Salir"
    ...
    case opcion of
        ...
        "2" -> do
            ...
            case buscarVehiculo placaVehiculo parqueadero of
                            Just vehiculo -> do
            --                  aqui poner tiempo actual, antes se restaba el tiempo del vehiculo
                                let tiempoTotal = tiempoEnParqueadero vehiculo tiempoActual
                                putStrLn $ "El vehículo con placa " ++ placaVehiculo ++ " está saliendo del parqueadero."
...
--
        "4" -> do
             putStrLn "Parqueadero:"
             printList parqueadero
             cicloPrincipal parqueadero
